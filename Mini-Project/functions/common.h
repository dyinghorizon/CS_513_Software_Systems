...
bool get_account_details_by_customer_id(int customerID, struct Account *account);


// =====================================================

// Add this line at the top of the file
extern struct Employee loggedInEmployee;
...
bool employee_manager_login_handler(int connFD, struct Employee *ptrToEmployee, bool isManager)
{
    ...
    if (writeBytes == -1)
    {
        perror("Error writing welcome message to client!");
        return false;
    }

    // Prompt for login ID
    bzero(writeBuffer, sizeof(writeBuffer));
    strcpy(writeBuffer, "Enter your login ID: ");
    writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
    if (writeBytes == -1)
    {
        perror("Error writing LOGIN_ID_PROMPT to client!");
        return false;
    }
    ...
    char loginID[30];
    strcpy(loginID, readBuffer);

    // Prompt for password
    bzero(writeBuffer, sizeof(writeBuffer));
    strcpy(writeBuffer, "Enter your password: ");
    writeBytes = write(connFD, writeBuffer, strlen(writeBuffer));
    if (writeBytes == -1)
    {
        perror("Error writing PASSWORD_PROMPT to client!");
        return false;
    }
    ...
    int employeeFileDescriptor = open("records/employee.bank", O_RDONLY);
    ...
    {
        if (strcmp(employee.login, loginID) == 0 && strcmp(employee.password, hashedPassword) == 0)
        {
            *ptrToEmployee = employee;
            loggedInEmployee = employee; // Initialize the global loggedInEmployee variable
            loginSuccess = true;
            break;
        }
    }
    ...
    {
        bzero(writeBuffer, sizeof(writeBuffer));
        writeBytes = write(connFD, "Invalid login credentials. Closing the connection to the server now!\n", strlen("Invalid login credentials. Closing the connection to the server now!\n"));
        if (writeBytes == -1)
        {
...
bool get_loan_application_details(int loanApplicationID, struct LoanApplication *loanApplication)
{
    ...
    {
        if (loanApplication->loanApplicationID == loanApplicationID)
        {
...
bool update_loan_application(int loanApplicationID, struct LoanApplication *loanApplication)
{
    int loanFileDescriptor = open("records/loan.bank", O_RDWR);
    if (loanFileDescriptor == -1)
    {
        perror("Error opening loan file!");
        return false;
    ...
    off_t offset;
    while ((offset = lseek(loanFileDescriptor, 0, SEEK_CUR)) != -1 && read(loanFileDescriptor, &tempLoanApplication, sizeof(struct LoanApplication)) > 0)
    {
        if (tempLoanApplication.loanApplicationID == loanApplicationID)
        {
            lseek(loanFileDescriptor, offset, SEEK_SET);
            write(loanFileDescriptor, loanApplication, sizeof(struct LoanApplication));
            close(loanFileDescriptor);
            return true;
        }
    }
...
{
    struct Account account;
    if (!get_account_details_by_customer_id(customerID, &account))
    {
        return false;
    }
    ...
    ssize_t writeBytes = write(accountFileDescriptor, &account, sizeof(struct Account));
    if (writeBytes == -1)
    {
...
bool get_account_details_by_customer_id(int customerID, struct Account *account)
{
    ...
    {
        if (account->owners[0] == customerID || account->owners[1] == customerID)
        {
            close(accountFileDescriptor);
            return true;
        }
